...
try {
  ...
  if ($res = $conn->query($sql)) {
      ...
      if ($res->fetchColumn() > 0) {
        foreach ($conn->query("SELECT isbn,name,author,count,remaining,edition,price,subject,reference,department,row from book where name LIKE '%$book2%' and author LIKE '%$author2%' and isbn LIKE '%$code2%'") as $row)
        {
